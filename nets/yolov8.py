...
class Conv(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)
        # print(f"x min: {torch.min(x)} - x max: {torch.max(x)} ")
        # print(x)
        x = self.norm(x)
        # print(f"x min: {torch.min(x)} - x max: {torch.max(x)} ")
        # print(x)
...
class DarkNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        p2 = self.p2(p1)
        p3 = self.p3(p2)
        # print(f"min p3: {torch.min(p3)} -  max P3: {torch.max(p3)}")
        p4 = self.p4(p3)
        p5 = self.p5(p4)
        return p3, p4, p5
...
class DarkFPN(torch.nn.Module):
    def __init__(self, width, depth):
        ...
        self.fn1 = torch.nn.quantized.FloatFunctional()
        self.fn2 = torch.nn.quantized.FloatFunctional()
        self.fn3 = torch.nn.quantized.FloatFunctional()
        self.fn4 = torch.nn.quantized.FloatFunctional()

    def forward(self, p3, p4, p5):
        p4 = self.h1(self.fn1.cat([self.up(p5), p4], 1))
        p3 = self.h2(self.fn2.cat([self.up(p4), p3], 1))
        p4 = self.h4(self.fn3.cat([self.h3(p3), p4], 1))
        p5 = self.h6(self.fn4.cat([self.h5(p4), p5], 1))
        return p3, p4, p5
...
class QAT(torch.nn.Module):
    ...
    def __init__(self, model):
        ...
        self.nc = self.model.head.nc if hasattr(self.model, "head") else  1
        self.no = self.model.head.no if hasattr(self.model.head, "no") else  5
