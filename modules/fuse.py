...
def _fuse_modules(model, fuse_patterns):
    """
    ...
    for pattern in fuse_patterns:
            ...
            while i <= len(child_names) - len(pattern):
                ...
                for j, name in enumerate(pattern):
                    ...
                    if name == "bn" and not (isinstance(child, nn.BatchNorm2d) or isinstance(child, nn.BatchNorm1d)):
                        match = False
                        break
                    if name == "relu" and not isinstance(child, nn.ReLU):
                        match = False
                        break
                if match:
        ...
        for module, names, fused_class in modules_to_fuse:
            ...
            # Add quantization parameters based on the fused class
            if fused_class in [ConvBnReLU, ConvBn, LinearBnReLU, LinearBn]:
                # These classes need is_fuse_bn parameter
                fused_args.extend([
                    "MinMaxObserver",    # observer_w_name
                    "UniformQuantizer",  # quantizer_w_name  
                    "MinMaxObserver",    # observer_a_name
                    "UniformQuantizer",  # quantizer_a_name
                    True,                # w_symmetric
                    True,                # a_symmetric
                    True                 # is_fuse_bn
                ])
            ...
            for n in names[1:]:
                del module._modules[n]
    return model

def _fuse_modules_trace(model, fuse_patterns):
    """
    ...
    try:
        ...
        return _fuse_modules(model, fuse_patterns)
...
def fuse_modules_unified(model, fuse_patterns, is_trace=False):
    """
    Unified fuse function. If is_trace is True, use tracing logic; else, use direct fuse logic.
    """
    if is_trace:
        return _fuse_modules_trace(model, fuse_patterns)
    else:
        return _fuse_modules(model, fuse_patterns) 